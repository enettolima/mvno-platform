...
class Mvno {
  /**
  * Method to register a new mvnos
  *
  * Add a new mvnos
  *
  * @url POST register
  ...
  */
  function register($request_data) {
    ...
    //$mvnos->insert();
    $result = $db->mvno()->insert($data);
    if ($result) {
      //Preparing response
      $response = array();
      $response['code'] = 201;
      $response['message'] = 'Mvnos has been created!';
      $response['id'] = $result['id'];
      natural_set_message($response['message'], 'success');
      return $response;
    } else {
      $error_message = 'Mvnos could not be created!';
  ...
  /**
  * Method to fecth Mvno Record by ID
  *
  * Fech a record for a specific mvno
  * by ID
  *
  * @url GET byID/{id}
  * @smart-auto-routing false
  ...
  * @param int $id Mvno to be fetched
  * @return mixed
  */
  function byID($id) {
    //If id is null
  ...
  */
  function fetchAll() {
    ...
    if(count($q) > 0) {
      ...
      natural_set_message('Mvno not found', 'error');
      throw new Luracast\Restler\RestException(404, 'Mvno not found');
    }
  }

  /**
  * Method to Update mvno information
  *
  * Update mvno on database
  *
  * @url PUT update
  * @smart-auto-routing false
  *
  ...
  */
  function update($request_data) {
    ...
    if($q){
      if($q->update($request_data)){
        $response['code'] = 200;
        $response['message'] = 'Mvno has been updated!';
        natural_set_message($response['message'], 'success');
      }else{
        //Could not update record! maybe the data is the same.
        $response['code'] = 500;
        $response['message'] = 'Could not update Mvno at this time!';
  ...
  /**
  * Method to delete a mvno
  *
  * Delete mvno from database
  *
  * @url DELETE delete
  * @smart-auto-routing false
  *
  ...
  */
  function delete($id) {
    ...
    if($q && $q->delete()){
      $response['code'] = 200;
      $response['message'] = 'Mvno has been removed!';
      natural_set_message($response['message'], 'success');
      return $response;
    }else{
      $response['code'] = 404;
      $response['message'] = 'Mvno not found!';
      natural_set_message($response['message'], 'error');
      throw new Luracast\Restler\RestException($response['code'], $response['message']);
      return $response;
    }
